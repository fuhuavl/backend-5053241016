...
func Sapa(nama string) string {
	return fmt.Sprintf("Halo, %s! Selamat datang di kelas Backend. Semangat terus dan semoga selalu konsisten", nama)
}
...
func CetakInfo(nama string) string {
	return fmt.Sprintf("Informasi Mahasiswa\nNama: %s\nNRP: %s\nGo Version: %s", nama, NRP, runtime.Version())
}
